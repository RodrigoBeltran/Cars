...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(player, false) then
			...
			-- if(menuWarning == false) then
			-- 	menuWarning = true
			-- 	exports['okokNotify']:Alert("", "To open CARMENU, press L", 3000, 'info')
			-- end
...
RegisterNetEvent('seatbelt:seatbelt')
AddEventHandler('seatbelt:seatbelt', function()
	local player = GetPlayerPed(-1)
	local vehicle = GetVehiclePedIsIn(player, false)
	...
	if(PedVehIsCar)then
		...
		if seatbeltIsOn then
			...
			exports['okokNotify']:Alert("", "Belt on", 3000, 'info')
			-- TriggerServerEvent('esx_mole_misiones:PlayOnSource','buckle', 0.9)
		else
			exports['okokNotify']:Alert("", "Belt removed", 3000, 'info')
...
Citizen.CreateThread(function ()
	...
	while true do
		...
		if seatbeltIsOn then 
			DisableControlAction(0, 39, true)  -- Disable exit vehicle when stop
			DisableControlAction(39, 39, true) -- Disable exit vehicle when Driving
		end
		-- Vehicle Seatbelt
		if IsPedInAnyVehicle(player, false) then
...
--[[function GetVehicleModificatorPercentageByKm(vehicle)
	...
	--------------[[CAR LOCK]]--------------
	RegisterNetEvent("vehiclelock:toggleVehLock")
	AddEventHandler("vehiclelock:toggleVehLock", function()
		ToggleVehicleLock()
	end)
	function ToggleVehicleLock()
		...
		if DoesEntityExist(car) then
			ESX.TriggerServerCallback('dd_carkey:fetchCarKey', function(hasKey)
					if hasKey then
	...
	function LockToggleEffects(car,locked)
		...
		if locked then
			...
			exports['okokNotify']:Alert("VEHICLE", "Unlocked", 3000, 'success')		
		elseif not locked then
			SetVehicleDoorsLocked(car, 2)
			PlayVehicleDoorCloseSound(car, 0)
			PlaySoundFromEntity(-1, "Remote_Control_Close", car, "PI_Menu_Sounds", 1, 0)
			exports['okokNotify']:Alert("VEHICLE", "Locked", 3000, 'success')	
		end
